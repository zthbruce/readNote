...
> 说到Join等或导致Shuffle Dependency算子，那必须从ReduceBuKey和GroupByKey这两个算子说起，这两个算子属于两个功能的算子，虽然都有聚合运算，其底层都来源于combineByKeyWithClassTag这个方法。

> 由于combineByKeyWithClassTag如此重要，我们先祭出其源码
> 
...
> Spark中Shuffle很重要的组件为：
1. 分区器(partitioner)
2. 序列化(serializer)
3. 聚合器(aggregator)
4. 排序器(keyOrdering)
5. map端聚合开关(mapSideCombine)
> shuffle依赖于这些组件

> 该函数中最重要的就是aggregator，该类三个聚合的参数都喂给该类：
    > 其中createCombiner,mergeValue,mergeCombiners是极其重要的三个元素：
    > combiner是容器C，mergeValue将v整合进入C, mergeCombiners: 将容器C进行整合成一个容器

### aggregator
> 这也算是算子的鼻祖之一，我们先从aggregator讲起。
> aggregator通过一个ExternalAppendOnlyMap的数据结构进行合并。
> aggregator在实现ShuffleDependency是发挥了大用处，我们看看Shuffle Write和Shuffle Read中的体现：
...
> aggregator有两个非常重要的方法，combineValuesByKey和combineCombinersByKey，map端聚合write都是使用combineValuesByKey，而reduce端的combineCombinersByKey则根据map端是否进行combine选择使用哪种方法。

> 使用这两个方法实现了map端和reduce端的聚合，那究竟是怎么实现聚合的呢？其实我们可以想象到map这种结构，map这种结构天生就是为聚合而生的。
> 先看看combineValuesByKey方法，combineCombinersByKey方法也类似
...
> 可见其内部使用了一种ExternalAppendOnlyMap的数据结构(该数据结构跟ExternalSorter中的partitionedAppendOnyMap相似，在hash shuffle中直接使用ExternalAppendOnlyMap，后来出现的sort-shuffle则采用了ExternalSorter中的数据结构)，只不过其是用来做聚合运算使用的，ExternalAppendOnlyMap的聚合原理和普通的map没什么区别，但是当数据量大到内存装不下时，就需要进行spill操作。spill操作之前，对key的hash值进行排序(不是为了真正的排序，更多的应该是为了分区，如何将分区后的数据放在一块，只要按照hash值相同的放在一块即可)

> External这个词就表示了需要进行外部排序(即聚合后的数据既在内存中出现，又在文件中)，insertAll结束之后，会进行merge，即进行分区文件的合并，实际的流程为
1. ExternalAppendOnlyMap不断读入数据，然后写入内部的SizeTrackingAppendOnlyMap进行不断聚合
2. 当SizeTrackingAppendOnlyMap内存达到阈值时，按照partition值排序，然后进行溢写到磁盘
3. 全局归并：
我们将会调用iterator 方法，在这里是真正完成聚合的关键所在。iterator 返回了一个基于内存中currentMap 和DiskIterator 两部分数据的多路归并迭代器。这个迭代器，每次在调用next 方法的时候都会在内部的优先级队列（按每个迭代器hash值作为比较对象的最小堆），寻找最小的hash值且key值相等的所有元素（因为我们每个map 都是排序过的，所以这总能实现），进行merge，将所有符合要求的元素merge完成后返回。这样便完成了最终的聚合操作。
...
> func为聚合函数，对于wordCount来说，就是使用v=v+v，mergeValue和mergeCombiner都一样

### groupByKey
> groupByKey是将相同key值的元素聚合的操作，该操作代价花销较大，如果是用来实现聚合操作，那么尽量使用reduceByKey等算子。
...
> 从源码中也可以看出来
1. mapSideCombine为false，源码里写的原因为：1. 首先即使做了combine，并不会减少map端的数目 2. 而且combine得使用ExternalAppendOnlyMap这种数据结构进行聚合，会增大GC开销，降低性能。

2. 使用了一种CompactBuffer(是一种迭代器)的combiner用于聚合元素，对于reduceByKey来说，聚合运算是自己传入的func，但是对groupByKey来说，是方法内部定义的combiner,mergeValue,mergeCombiners，由此可见，实际上和reduceByKey的流程相似，但有区别：区别1为mapSideCombine; 区别2为自定义的combiner(CompactBuffer(v)类似于ArrayBuffer这样的结构吧);区别3为使用的func不同：reduceByKey使用的是result = result+record.value, groupByKey为result = result ++ record.value

### distinct
> 原理类似于reduceByKey，只不过使用聚合时使用的func，如果Map中没有该key则加入，否则则丢弃
为result = result == null? record.value : result；

> 可以使用map端聚合，是一个去重操作，可以提前在map端去重，然后将数据保存下来。

### cogroup
> 原理上为：
> 每个key -> Array[ArrayBuffer(), ArrayBuffer()]

> task 首先 new 出一个 Array[ArrayBuffer(), ArrayBuffer()]，ArrayBuffer() 的个数与参与 cogroup 的 RDD 个数相同。func 的逻辑是这样的：每当从 RDD a 中 shuffle 过来一个 <Key, Value> record 就将其添加到 hashmap.get(Key) 对应的 Array 中的第一个 ArrayBuffer() 中，每当从 RDD b 中 shuffle 过来一个 record，就将其添加到对应的 Array 中的第二个 ArrayBuffer()。

> CoGroupedRDD => MappedValuesRDD 对应 mapValues() 操作，就是将 [ArrayBuffer(), ArrayBuffer()] 变成 [Iterable[V], Iterable[W]]。

### intersection
> Shuffle过程和cogroup一模一样，只是需要对cogroup后的数据集进行过滤，两边都存在数据的留下，否则过滤。

### join
> Shuffle过程和cogroup一模一样，只是对cogroup之后对应的两个ArrayBuffer进行遍历组合，可能需要做一遍筛选。
> 如果是 inner join(join)，只留下两边均有数据的进行组合
> 如果是left join，则保留左边不为空的数据进行组合
> 如果是right join，则保留右边不为空所有数据进行组合
> 如果是 outer join，对每个key进行组合

### sortByKey
> sortByKey比较特殊的一点就是，在reduce端仍然使用ExternalSort在每个分区内根据key值进行排序，又因为使用range partitioner的方式进行分区，所以可以达到总体有序

> 注意点：
sortByKey实际上在进行rangePartition的时候，已经产生了一个action动作(做收集的动作)

### coalesce(numPartitions, shuffle = true)
> 当父RDD的分区为N，子RDD的分区为M，那么当 N > M时，重新分区且对key值没有要求时，可以采用合并的方式进行重分区，而不需要进行shuffle，该方式产生的父RDD和子RDD的依赖关系为窄依赖。
...
## Spark任务和计算资源调度
> Spark任务调度的模块主要分为：TaskScheduler，schedulerbackend，DAGScheduler
> 三个都是由driver的SparkContext产生的。
> 将三者分离是为了功能解耦，维护和分配方便。
> dagScheduler功能在于划分stage和提交task(划分stage，提交stage，提交task)；
> taskScheduler功能在于分发task；(将task分配到哪个Executor执行)
> schedulerbaclend功能在于资源分配(Executor/Container)，并且在分配的Executor中启动Task(需要通信和分发)；

> 在Spark中，taskScheduler和schedulerbackend是SparkContext中同时生成的。
> 源代码如下：
    val (sched , ts) = SparkContext.createTaskScheduler(this, master) // 这里的master是"spark.master"参数的值，
...
> 其中master在spark-submit时，可以采用--master的方式进行设置，常见的有local[*]，yarn-client,yarn-cluster,mesos等。

> 将ScheduleBackEnd类的接口的UML图设置为如下：
![schedulerBackEnd](pic/SchedulerBackEnd.png)

### DAGScheduler
#### DAG(有向无环图)
> 如果一个图是有向图，且无法从一个顶点经过若干条边返回到该顶点，则称为有向无环图。
> 有向无环图未必可以转化为树，但树一定是有向无环图。
> Spark中用一个DAG来表示一个job。
#### 功能
> 这个DAGScheduler
1. 提交job
2. 划分Stage
...
> 其实从原理上，提交是环环相扣的，job的提交由Stage划分和提交Stage两个步骤完成，而提交Stage则由提交Task完成。

> 提交Stage原理：
　　从最后那个Stage开始的。如果当前stage已经被提交过，处于waiting或者waiting状态，或者当前stage已经处于failed状态则不作任何处理，否则继续提交该stage。 
...
### TaskScheduler
> TaskScheduler本质上来说是一个trait类，trait类本质上类似java的接口类，需要自己实现。不同的master下实现如下：
1. Standalone模式：TaskSchedulerImpl
2. Yarn Client模式：YarnScheduler
3. Yarn Cluster模式：YarnClusterScheduler
4. Mesos模式：TaskSchedulerImpl

> 其功能承接DAGScheduler中调用taskScheduler的submitTasks。所以taskScheduler和schedulerBackEnd需要在DAGScheduler之前生成。


### SchedulerBackend
> 同样也是一个trait类，其继承于CorseGrainedSchedulerBackEnd类，该类实现了两个接口，ExecutorAllocationClient和SchedulerBackend，由此可知，Spark即可以与cluster Manager通信，请求和kill需要的Executor，也可用于请求计算资源。同时还继承了RPC通信的功能。
需要根据不同的master选择不同的实现。

1. Standalone模式：SparkDeploySchedulerBackend
2. Yarn Client模式：YarnClientSchedulerBackend
3. Yarn Cluster模式：YarnClusterSchedulerBackend
4. Mesos模式：MesosSchedulerBackend

#### 功能
每一个SchedulerBackend的具体实现起到的作用基本相同，就是向当前等待分配计算资源的Task分配计算资源（即Executors），并且在分配的Executor上启动Task，完成计算的资源调度过程。

### Task提交过程
> Task的提交过程决定了惰性求值的，从RDD依赖来划分Stage，然后提交Stage(需要将TaskSet全部提交了)，然后进入下个Stage

#### DAGScheduler#submitMissingTasks(提交Task)
> 提交Task是通过DAGScheduler的submitMissingTasks方法进行提交的。
> 在提交Stage时，对于不同的ShuffleMapStage和ResultStage，有不同的处理逻辑。最终根据Stage对于的rdd partition生成tasks组，然后通过TaskScheduler.submitTasks方法，将tasks生成的TaseSet进行提交。 
> taskSet是对一组task的封装，其结构如下：
> 
...
#### TaskSchedulerImpl#submitTasks(提交任务)
在该方法中，根据taskSet生成一个TaskSetManager类型对象，负责这部分taskSet的运行。

> 向资源调度池提交任务，schedulableBuilder.addTaskSetManager(manager, manager.taskSet.properties)。schedulableBuilder一般有FIFO和FAIR两种模式。

> backend.reviveOffers()：// 向schedulerBackend申请资源

#### CoarseGrainedSchedulerBackend#reviveOffers(申请资源)
> 该方法是在taskScheduler的submitTasks中最后一个动作。其源代码比较简单:
> 
  override def reviveOffers() {
    driverEndpoint.send(ReviveOffers)
  }

> 意思为向驱动发送资源申请，schedulerBackEnd此时闪亮登场
> 首先，driverEndPoint实际上是继承了NettyRpcEnv的send方法

#### NettyRpcEnv#send(RPC调用,也是一种通信手段)
> 在Spark1.6之前，使用Akka作为通信的方式比较多，在之后，netty完全取代了Akka的地位，变成了通信主导。
> 有发送，必然有地方接受， 接收的地方就是scheduleBackEnd的receive方法

#### CoarseGrainedSchedulerBackend#receive(接收申请)
> 源码如下：
...
#### CoarseGrainedSchedulerBackend#makeOffers(发出资源)
> 该方法首先会获取哪些Executor是活着的，然后申请Executor资源，封装成workerOffer资源队列，进行干活。
> 然后执行launchTasks方法，该方法接收的参数是schedulerBackend分配的计算资源。

#### TaskSchedulerImpl#resourceOffers(为task分配节点上的资源)
> 资源生成好封装成WorkerOffer类型的队列后，就开始进入SchedulerBackend中，由SchedulerBackend分配这些资源。 

#### TaskSchedulerImpl#resourceOfferSingleTaskSet(分配单个TaskSet计算资源)
> 执行taskSet中包含的所有task。首先遍历分配到的executor，如果当前executor中的cores个数满足配置的单个task需要的core数要求(该core数由参数spark.task.cpus确定，默认值为1)，才能在该executor上启动task。

#### TaskSetManager#resourceOffer
>上面方法中，将executor分配给taskSet之后在这个方法中，最终启动一个个task，并未启动的task生成task描述对TaskDescription。isZombie的值在上面已经讲过了，如果为false，则表示当前TaskSetManager中的task没有全部执行成功，可以继续执行剩余task。 
...
## Spark任务计算
> 上一部分，讲述了Task是如何进行提交的，最后使用的方法是：CoarseGrainedSchedulerBackend#launchTasks方法
...
>　Driver和Executor之间仍然使用Netty进行通信，在Driver端send一个LaunchTask的消息后，在Executor端会对应有一个receive方法接收消息。
...
> 将提交的task封装成taskRunner，然后交由线程池运行。
...
## Spark Shuffle参数调优
> Spark Shuffle是非常重要的一个过程，同时也是非常损耗内存和硬盘的一个过程，所有shuffle调优是一个非常重要的内容。
### spark.shuffle.file.buffer
1. 默认值：32k
2. 参数说明：该参数用于设置shuffle write task的BufferedOutputStream的Buffer缓冲大小。将数据写入磁盘之前，会先写入buffer缓冲(一般的ExternalSorter中会有两种数据结构，partitionedAppendOnlyMap和partitionedPairedBuffer，当需要进行combine时，则采用partitionedAppendOnlyMap，不然则采用partitionedPairedBuffer)，待缓冲写满之后，才会溢出。
3. 调优：如果工作可用的内存资源较为充足的话，可以适当增加这个参数的大小(比如64k)，从而减少shuffle溢写的次数，也可以减少最终合并的次数，减少磁盘IO次数，进而提升性能。在实践中发现，合理调节该参数，性能会有所提高。

### spark.reducer.maxSizeInFlight
1. 默认值：48m
2. 参数说明：该参数用于设置shuffle read task的buffer缓冲大小，这个大小决定了每次能拉多少数据。
3. 调优：如果内存较为充足，可适当提高该参数的大小(比如96m)，从而减少拉取数据的次数，可以减少网络传输的次数，从而提高性能。

### spark.shuffle.io.maxRetries
1. 默认值：3
2. 参数说明：shuffle read task从shuffle write task所在节点拉取属于自己的数据时，如果因为网路异常导致拉取失败，时会自动进行重试的，重试的参数默认为3，当超过了最大重试次数之后，就会导致task失败
3. 调优：对于shuffle任务比较耗时的作业，建议增加重试的最大次数(比如60次)，以避免gc过程或者网络不稳定导致的数据拉取失败。
在实践中，对于超大数据量(数十亿~上百亿)的shuffle过程，调节该参数可以极大的增加稳定性(Spark任务失败的代价比较大，容错机制，有checkout和缓存(cache)机制)

### spark.shuffle.io.retryWait
1. 默认值：5s
2. 参数说明：具体解释同上，该参数代表了每次重试拉取数据的等待间隔，默认是5s
3. 调优建议：建议加大间隔时长(比如60s)，增加shuffle的稳定性


### spark.shuffle.memoryFraction
1. 默认值： 0.2
2. 参数说明：该参数代表了Executor内存中，分配给shuffle read task进行聚合运算的内存比例，默认是20%，剩余的(计算占20%，缓存占60%)
3. 调优建议：如果内存充足，而且很少使用缓存和持久化操作，则可以适当提高该部分的内存比例，给shuffle read的聚合操作更多内存，以避免内存不足导致聚合过程中频繁读写磁盘。在实践中，合理使用可以提高性能
