...
My97DP.prototype = {
    ...
    _dealFmt: function() {
        ...
        function _(_) {
            var $ = (_ + "").slice(1, 2);
            $dp.has[$] = _.exec($dp.dateFmt) ? ($dp.has.minUnit = $,
            true) : false
        }
    },
...
My97DP.prototype.checkAndUpdate=function() {
    var _ = $dp.el
      , A = this
      , $ = $dp.el[$dp.elProp];
    if ($dp.errDealMode >= 0 && $dp.errDealMode <= 2 && $ != null ) {
...
My97DP.prototype.initBtn = function() {
    $d.clearI.onclick = function() {
        if (!callFunc("onclearing")) {
            $dp.valueEdited = 0;
            $c.update("");
    ...
    if (this.checkValid($tdt)) {
        ...
        $d.todayI.onclick = function() {
            $dt.loadFromDate($tdt);
            day_Click()
           $dp.el[$dp.elProp] = $dp.language=='zh_CN'?'至今':'Present';
           // $dp.hide()
