...
## hdfs常用的压缩模式
### 背景
> hdfs作为一个较通用的海量数据文件系统，每次运算可能都会需要处理大量数据，我们会在Hadoop系统中对数据进行压缩处理来优化磁盘使用率，提高数据在磁盘和网络中的传输速度(减少文件大小很重要，因为需要通过网络传输的方式进行)

> 使用压缩的好处:
1. 节省数据占用的磁盘空间。
2. 加快数据在磁盘中和网络中的传输，从而提高系统的处理速度。

> hadoop对于压缩格式是自动识别，如果有相应的压缩格式的扩展名：lzo,bz2,gz,snappy等。
> hadoop会根据压缩格式的扩展名自动选择相对应的解码器来解压数据，此过程完全是 Hadoop 自动处理，我们只需要确保输入的压缩文件有扩展名

### 常用的压缩方法
1. gzip压缩
* 优点:
    (1) 压缩率比较高，而且压缩/解压速度也比较快
    (2) hadoop本身支持，在应用中处理gzip格式的文件就跟直接处理文本文件一样
    (3) 有hadoop ntaive库
